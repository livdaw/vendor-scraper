// Auto-generated scraper loader - do not edit directly
// This file is loaded by the "Scrape Page" bookmarklet
(async function(){
// ── Status Bar UI ──
...
if(PLATFORM==="shopify"){
  ...
  var collName=document.querySelector("h1,.collection-title,.page-title");
  ...
  shopProds.forEach(function(sp){
    if(count>=MAX)return;
    var p=extractShopifyProduct(sp,collName);
    if(p&&p.title){
  ...
  for(var i=0;i<allUrls.length;i++){
    ...
    try{
      var doc=await fetchDoc(allUrls[i]);
      var p=extract(doc,allUrls[i]);
      if(p&&p.title){
        window.__scrapedProducts.push(p);
      }
...
console.log("Scraper complete: "+total+" products");
})();
